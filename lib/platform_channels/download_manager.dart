...
class DownloadManager {
  ...
  factory DownloadManager(BuildContext context) {
    if (_instance == null) {
      ...
      _instance = new DownloadManager.private(downloadEventChannel,
          downloadMethodChannel, context, AppSharedStateContainer.of(context));
    }
    return _instance;
  }

  @visibleForTesting
  DownloadManager.private(this._downloadEventChannel,
      this._downloadMethodChannel, this._context, this.appWideState);
  ...
  Future<Video> downloadFile(Video video) async {
    ...
    requestArguments.putIfAbsent("fileName",
  ...
  DownloadStatus parseDownloadEvent(Map event) {
    ...
    if (AppSharedStateContainer.of(_context)
                .appState
                .currentDownloads[videoId] ==
            null &&
...
//          );
  ...
  static String getFileNameForVideo(
      String videoId, String videoUrl, String videoTitle) {
