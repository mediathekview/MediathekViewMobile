...
import 'package:flutter_ws/widgets/list/metadataBar.dart';
        ...
        children: <Widget>[
          isExtendet == false
              ...
              : new Row(
                  ...
                  children: <Widget>[
                    isLivestreamVideo(widget.video) ? new Container() : new Container(
                        key: new Key(uuid.v1()),
                      ...
                      isLivestreamVideo(widget.video) ? new Container() : new Switch(
                          key: new Key(uuid.v1()),
                          ...
                          onChanged: (newSwitchValue) {
                            ...
                            if (newSwitchValue == true && status == null) {
                              print("Triggering download for video with id " +
                                  widget.video.id);
                              onDownloadRequested();
                              return;
                            }
                            ...
                            if (isAlreadyDownloaded) {
                              ...
                              databaseManager.delete(videoEntity.id).then((id) {
                            ...
                            if (status != null &&
                              ...
                              print("Canceling download for video with id " +
                                  widget.video.id);
...
//        updatedStatus.status.toString());
    ...
    if (updatedStatus.status == DownloadStatusText.STATUS_SUCCESSFUL) {
      ...
      print("Download Card Body: Download with id" + widget.video.id + " is successfull");
...
//      });
    ...
    if (updatedStatus.status == DownloadStatusText.STATUS_FAILED) {
      switchValue = false;
...
//        switchValue = true;
  ...
  void onDownloadRequested() {
    downloadManager.downloadFile(widget.video).then(
        (video) => print("Downloaded request successfull"), onError: (e) {
  ...
  bool isLivestreamVideo(Video video){
    return video.url_video.substring(video.url_video.lastIndexOf(".")) == ".m3u8";
  }
}
